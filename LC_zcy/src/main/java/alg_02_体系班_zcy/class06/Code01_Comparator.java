...
//			return 0;
    ...
    public static void main(String[] args) {
        ...
        // 使用集合自己api(sort)方法
        studentList.sort(new IdShengAgeJiangOrder()); // 排序时间复杂度:O(N*logN)
        ...
        // 1)使用有序表,需要在构造方法中传入定义排序规则(自己定义)
        // 2)TreeMap会根据比较字段来判断两个对象是否一样
        //   若比较字段一样,则认为是相同的对象,不重复加入的(注意:直接是不加入,而不是加入再覆盖)
        // 3)这里若是使用age作为比较字段,则每个student信息都会保存下来
