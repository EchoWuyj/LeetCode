    ...
    public static int calculate(String s) {
        ...
        while (i < s.length()) {
            ...
            if (s.charAt(i) != ' ') {
                ...
                if (preSign == '+') {
                    // 遇到 '+'，只是将数字压栈，不进行运算
                    // 将 +3|+21| 压入栈中，即 stack：3 21
                    stack.push(num);
                } else if (preSign == '-') {
                    // 遇到 '-'，只是将数字压栈，不进行运算
                    // 将 -2 压入栈中，即 stack：3 21 -2
                    stack.push(-num);
                } else if (preSign == '*') {
                    // 获取栈顶元素 -2
                    int tmp = stack.pop();
                ...
                // if else if 分支，只会执行其中一个分支
                // 判断一个运算符号，将相应运算结果压栈后，数字和运算符之间可能存在空格
                // 故需要处理空格，将其跳过。比如：" +3 + 5 / 2 "
                ...
                // 本质：先确定 preSign，再去通过 if else 判断，确定 preSign 何种运算
