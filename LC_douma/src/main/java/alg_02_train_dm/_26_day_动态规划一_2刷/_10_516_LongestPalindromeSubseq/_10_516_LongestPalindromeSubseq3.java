package alg_02_train_dm._26_day_动态规划一_2刷._10_516_LongestPalindromeSubseq;
...
public class _10_516_LongestPalindromeSubseq3 {
    ...
    public int longestPalindromeSubseq(String s) {
        ...
        // 定义状态关键：
        // 1.状态参数 => [start,i] 区间范围 => 定义为 [i,j]
        // 2.状态值 => 状态值和所求问题保持一致 => 最长回文子序列的长度

        // 状态
        // dp[i][j] 表示在区间 [i,j] 中最长回文子序列的长度
        ...
        for (int i = n - 2; i >= 0; i--) {
            for (int j = i + 1; j < n; j++) {
                ...
                if (s.charAt(i) == s.charAt(j)) {
                    ...
                    // 2.若 s.charAt(i) != s.charAt(j)，两端不相等，则两端不同时包括，而是分别只包括一端
                    // 故 dp[i][j] = Math.max(dp[i + 1][j], dp[i][j - 1])
                    // 注意：这里不能直接跳过，i 和 j，dp[i][j] = dp[i + 1][j - 1]，这样是有遗漏解
    ...
    // KeyPoint 若代码性能不高，尝试取掉 i，进行状态压缩，看看能不能通过
    //          => 本题不能通过消除 i来进行状态压缩
}
