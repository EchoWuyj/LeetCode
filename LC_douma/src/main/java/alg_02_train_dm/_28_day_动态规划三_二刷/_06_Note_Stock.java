...
public class _06_Note_Stock {
    /*
        ...
        KeyPoint 注意：你不能同时参与多笔交易(你必须在再次购买前出售掉之前的股票)
        prices  7  1   5    3    6  4
        索引    0  1   2    3    4  5
                   ↑   ↑    ↑    ↑
        ...
        返回获得利润的最大值。
        KeyPoint 注意：这里的一笔交易指买入持有并卖出股票的整个过程，每笔交易你只需要为支付一次手续费
        prices  1  3  2  8   4   9
        ...
        KeyPoint 最优值问题 + 通过枚举方式求解 + 且存在重复值 => 动态规划
        ...
        其中，分析 i,k 都是一般意义上状态，具有通用性

        状态初始化
        dp[i][0][0] = 0：表示在第 i 天结束时，没有进行任何交易，也不持有股票，所以利润为 0

        KeyPoint 注意事项
        这里的'利润'指的是：在第 i 天结束时，获取的利润，不是从开始第 1 天到现在第 i 天累计的利润
        dp[i][0][0] = 0，只和 i 有关，只要第 i 天，不交易，且没有持有股票，其利润为 0

        dp[i][k][0]：表示在第 i 天结束时，经过 k次交易，且不持有股票：那么
            1.前一天不持股，然后今天休息，最大利润为：dp[i-1][k][0]
             KeyPoint 在买卖股票中，一次交易有两个动作
                      定义：买入操作算作一次交易，而卖出不算一次交易
                      故 k-1 原因：买入操作会使用一次交易
            2.前一天持股，然后今天卖出，最大利润：dp[i-1][k][1] + prices[i]
            => 注意：i 一般是和相邻 1 天 (i-1) 进行比较，而不是相邻的多天 (i-n)比较，
               当前状态是由相邻的前一个状态推导，不用间隔太多天，而 i-1 天再由 i-2 天推导
