...
public class _00_01_Note {
    /*
                    ...
                    1.预计算(预处理)：先一次性将数据预处理(计算，排序)，将预处理的结果暂存
                        ...
                        1.2 排序：对输入数据先排序
                                ...
                                2) 排序后，相同的元素会在一起，有的时候可以降低时间复杂度
                                   比如：排序去重复 nums[i] 和 nums[i-1] 相等 => continue
                                3) 注意：排序使用场景，有些情况不能使用排序
        ...
        3.经典查找算法 => 在解题步骤中，涉及在数组中，查找某个元素，都是属于查找算法
            3.1 线性查找
                for 循环遍历查找
            3.2 二分查找：
                前提：有序数组，不是有序，可以先排序
                单个索引 i => 直接二分查找
                两个索引 i 和 j => 1.先固定一个 i，通过二分查找找 j
                                  2.再通过 for 循环，遍历所有 i 位置
            ------------------------------------------------------------------
            空间换时间 => 空间复杂度一般是 O(n)
            3.3 哈希查找
                将数组建立映射，关键 key 和 value 定义，通过 map 的 key 查找 value

            3.4 堆查找
                局部有序
                查找堆顶 O(1)，维护堆 O(logn)
                数组第 k 个大元素
                数据结构设计：堆作为存储结构，新加入元素自动排好序

            3.5 辅助查找：栈(先进后出场景) -> 栈顶操作 O(1)
                                ↓
                          先遇到元素，暂时不知道怎么处理 或 当前元素处理依赖后面元素
                          则将当前元素临时存储(stack)，等遇到后面元素，再处理前面的元素
            ...
            3.6 双指针 -> 针对：线性数据结构，一般是数组
                1.快慢指针
                  slow 和 fast
                  关键：定义划分区间，[0,slow]，[slow,fast]，[fast,未处理数组区间]
                  链表找中点：slow 走一步，fast 走两步

                2.对撞指针
                  left 和 right 分别从数组两端向中间遍历
                  有些情况，需要数组有序，可以先排序，再从数组两端向中间遍历
