...
public class _04_136_single_number {
    ...
    public int singleNumber1(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            // 内层 for 循环遍历结束，isExist 还是 true，则 nums[i] 为只出现一次的数字
    ...
    public int singleNumber2(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            if (i == 0 && nums[i] != nums[i + 1]) return nums[i];
                ...
                // 判断:中间元素，和前后元素都要比较
                // KeyPoint bug 修复，i 需要 > 0，否则 i - 1 就是负数了
                //          => 最开始的 if 是有 2 个判断条件，但不一定都成立，故不能保证一定有 i > 0
                // 注意，if else 判断是多分支中选择一个执行，类似于多个 if，前一个 if 对于后面的 if 具有排他性
    ...
    public int singleNumber3(int[] nums) {
        if (nums.length == 1) return nums[0];
        // HashSet 去重，只能判断是否存在，但是无法判断出现的次数
        // HashMap 映射表 => 记录 num 的次数
        // key num
        // value count
