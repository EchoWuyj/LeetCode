...
public class _06_CompleteKnapsack2 {
    ...
    public int knapsackComplete(int[] w, int[] v, int capacity) {
        ...
        for (int i = 0; i <= capacity; i++) {
            // 区别于：dp[0][i] = (i >= w[0] ? v[0] : 0)
            // 若 dp[0][i] 不设置，默认值即为 0，可以不单独定义
            dp[0][i] = (i / w[0]) * v[0];
        }
        ...
        for (int i = 1; i < n; i++) {
            for (int j = 0; j <= capacity; j++) {
                ...
                for (int k = 0; k <= count; k++) {
                    ...
                    // 注意：完全背包涉及 for 循环，for 循环里面为的是获取最大值，而 dp[i][j] 并没有参与最大值的选择
                    // 即 dp[i][j] 可能被多次赋值，前期的较大值可能被后面较小值覆盖，不能保证 dp[i][j] 为最大值

                    dp[i][j] = Math.max(dp[i][j], k * v[i] + dp[i - 1][j - k * w[i]]);

                    // 为什么能将 dp[i-1][j] 替换成 dp[i][j]？
                    // 当 k = 0 时，dp[i][j] = Math.max(dp[i - 1][j], dp[i - 1][j])，
                    // 此时 dp[i][j] 已经记录了 dp[i - 1][j]，再使用 dp[i][j] 和 k * v[i] + dp[i - 1][j - k * w[i]] 比较，
                    // 从而获取最大值，赋值给 dp[i][j]
                }
            }
        }
