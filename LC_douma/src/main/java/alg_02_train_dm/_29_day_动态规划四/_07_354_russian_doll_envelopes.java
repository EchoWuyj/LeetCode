...
public class _07_354_russian_doll_envelopes {
    ...
    // 本质：找出二维数组的一个排列，使得其中有最长的单调递增子序列(需要保证两个维度都递增)
    // 就是 300 号算法题的变形体
    public int maxEnvelopes(int[][] envelopes) {
        ...
        // KeyPoint 补充说明：测试用例修改了数据规模

        // 俄罗斯套娃这道题的输入改掉了，之前的输入数据规模是 5000，动态规划 O(n^2) 方案是可以的
        // 现在的输入数据规模改为 10^5 ，所以时间复杂度为 O(n^2) 会超时了
    ...
    public int maxEnvelopes2(int[][] envelopes) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            //   注意：当前信封宽度肯定大于等于 res 中最后一个信封的宽度，因为是按照宽度升序排列的
