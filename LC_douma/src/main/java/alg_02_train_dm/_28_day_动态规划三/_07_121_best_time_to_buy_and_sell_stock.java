...
public class _07_121_best_time_to_buy_and_sell_stock {
    ...
    /*
        ...
        若是三维，应该如下：dp[i][1][1] = Math.max(dp[i - 1][1][1], dp[i - 1][0][0] - prices[i]);
        且 dp[i - 1][0][0] = 0，所以代码变为：dp[i][1][1] = Math.max(dp[i - 1][1][1],  - prices[i]);
        这个时候才可以将三维压成二维，即：dp[i][1] = Math.max(dp[i - 1][1], -prices[i]);
