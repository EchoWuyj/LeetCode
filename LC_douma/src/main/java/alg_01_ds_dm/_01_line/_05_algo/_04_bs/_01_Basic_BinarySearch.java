...
public class _01_Basic_BinarySearch {
    ...
    public static boolean contains(int[] nums, int target) {
        ...
        while (left <= right) {
            // left + right 可能会溢出，整数的最大值：2^31 - 1 = 2147483647
            // 这个 mid 计算公式 => 奇数个元素 mid 为中间位置，偶数个元素 mid 为偏左位置(向下取整原因)
            // => 推荐使用这种方式
        ...
        // left = right，mid = left = right，都没有找到 nums[mid] == target
        // => left > right：没有元素，返回 false
        return false;
    }

    // KeyPoint 方法二 基本二分查找(递归) => 了解
    // 时间复杂度：O(logn) => 取决于 target 比较的次数
    //                     => 给定长度为 n 数组，二分查找次数为 k = 1 + 1og2(n) 次
    // 空间复杂度：O(logn)
