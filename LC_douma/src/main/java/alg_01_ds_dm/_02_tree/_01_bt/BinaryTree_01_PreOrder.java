...
public class BinaryTree_01_PreOrder {
    ...
    // 前序遍历(非递归) => 栈
    // 时间复杂度：O(n), n 表示二叉树节点个数 => 遍历树的每个节点，针对操作进行分析，没有看具体代码
    // 空间复杂度：O(n)
    public static List<Integer> preOrder(TreeNode<Integer> root) {
        ...
        while (!stack.isEmpty()) {
            ...
            // 先加入右子节点，再加入左子节点
            // KeyPoint curr.right 为 null，则是没有必要放入 stack，同时也是不能放入 stack
            if (curr.right != null) stack.push(curr.right);
            if (curr.left != null) stack.push(curr.left);

            // 经过上面两步操作，cur 节点的左右子节点已经判断过了，while 循环结束，
    ...
    //  1 递归过程虽然没有直接申请额外的空间，但程序因为是递归的，需要申请系统栈，而系统栈是需要消耗空间的
    //    递归调用几次，则系统栈中压入几个栈帧，因此，系统栈使用的大小取决于树的高度
    //  2 树的高度需要分情况：最好(完全二叉树) => O(logn)
    //                       最差(斜树) => O(n)
    private static void preOrder(TreeNode<Integer> node, List<Integer> res) {

        // ArrayList<Integer> res = new ArrayList<>();
        // 用来收集结果的全局变量 res 不能定义在递归方法中，每次递归调用时都会创建一个 res
        // 最好的方式将 res 当做形参，传入到递归方法中，推荐使用，记住这种写法
