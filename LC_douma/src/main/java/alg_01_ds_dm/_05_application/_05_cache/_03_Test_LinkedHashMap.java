...
public class _03_Test_LinkedHashMap {
    ...
    private static void test2() {
        // KeyPoint
        // 1.LinkedHashMap 会维护 put 的键值对(KV)的相对顺序，输入顺序和显示顺序一致(双向链表可以维护节点之间的顺序)
        // 2.默认情况下 LinkedHashMap 是不会维护：最近使用/最久未使用的数据位置
        //   但可在构造方法中通过参数设置，将最近使用的键值对放在表头，最久未使用的键值对放在表尾
        ...
        /*
            ...
            默认情况下 LinkedHashMap 是不会维护：数据最近使用/最久未使用的位置
            但可在构造方法中通过参数设置，初始容量，装载因子，访问顺序(将最近使用的键值对放在表头，最久未使用的键值对放在表尾)
         */
    }
}
